...
from apps.core.views.telemetry import HealthCheckView, TelemetryDashboardView

urlpatterns = [
    # Health Check Probe (Readiness / Liveness)
    path("healthz/", HealthCheckView.as_view(), name="healthz"),
    # Observability & Telemetry Dashboard (Superuser/Staff Only)
    path("dev/telemetry/", TelemetryDashboardView.as_view(), name="telemetry"),
    ...
    path(
        "rdp-ui/",
        RedirectView.as_view(url="https://ui.radian.web.id", permanent=False),
        name="rdp-ui-landing",
    ),
    path(
        "rdp-ui/tokens/",
        RedirectView.as_view(url="https://ui.radian.web.id/docs/", permanent=False),
        name="rdp_ui_token_test",
    ),
    path(
        "rdp-ui/layout/",
        RedirectView.as_view(url="https://ui.radian.web.id/docs/", permanent=False),
        name="rdp_ui_layout_test",
    ),
    path(
        "rdp-ui/components/",
        RedirectView.as_view(url="https://ui.radian.web.id/docs/", permanent=False),
        name="rdp_ui_components_test",
    ),
]
...
try:
    ...
    urlpatterns += [
        ...
        path(
            "starter/layout/",
            RedirectView.as_view(url="/examples/layout/", permanent=True),
            name="starter_layout",
        ),
        path(
            "starter/components/",
            RedirectView.as_view(url="/examples/components/", permanent=True),
            name="starter_components",
        ),
        path(
            "starter/cli/",
            RedirectView.as_view(url="/docs/#cli", permanent=True),
            name="starter_cli",
        ),
        path(
            "starter/auth/",
...
try:
    ...
    urlpatterns += [
        ...
        path(
            "examples/htmx/contact/create/", ContactCreateView.as_view(), name="htmx-contact-create"
        ),
        path(
